...
class Presenter: NSObject {
	...
	var delayHide: TimeInterval? {
		...
		if case let .indefinite(_, minimum) = config.duration, let showDate = showDate {
	...
	private func showAnimation(completion: @escaping AnimationCompletion) {
		...
		animator.show(context: makeAnimationContext(), completion: completion)
	...
	func hide(animated: Bool, completion: @escaping AnimationCompletion) {
		...
		let action = {
			if let viewController = self.presentationContext.viewControllerValue() as? WindowController {
		...
		animator.hide(context: makeAnimationContext()) { _ in action() }
	...
	private func makeAnimationContext() -> AnimationContext {
		AnimationContext(messageView: view, containerView: maskingView, safeZoneConflicts: safeZoneConflicts(), interactiveHide: config.interactiveHide)
	}

	private func safeZoneConflicts() -> SafeZoneConflicts {
		guard let window = maskingView.window else { return [] }
		let windowLevel: UIWindow.Level = {
			if let vc = presentationContext.viewControllerValue() as? WindowController {
	...
	private func getPresentationContext() throws -> PresentationContext {
		func newWindowViewController(_ windowLevel: UIWindow.Level) -> UIViewController {
			let viewController = WindowController.newInstance(windowLevel: windowLevel, config: config)
...
#else
			if let rootViewController = _keyScene?.windows.first(where: { $0.isKeyWindow })?.rootViewController {
	...
	func install() {
		...
		if let windowController = presentationContext.viewControllerValue() as? WindowController {
			if #available(iOS 13, *) {
				windowController.install(becomeKey: becomeKeyWindow, scene: windowScene)
			} else {
				windowController.install(becomeKey: becomeKeyWindow)
			}
		}
