...
open class SwiftMessagesSegue: UIStoryboardSegue {
	...
	/// Specifies how the view controller's view is installed into the containing message view.
	public enum Containment {
		/// The view controller's view is installed for edge-to-edge display, extending into the safe areas to the device edges. This is done by calling `messageView.installContentView(:insets:)`
		/// See that method's documentation for additional details.
		case content

		/// The view controller's view is installed for card-style layouts, inset from the margins  and avoiding safe areas. This is done by calling `messageView.installBackgroundView(:insets:)`.
		/// See that method's documentation for details.
		case background

		/// The view controller's view is installed for tab-style layouts, inset from the side margins, but extending to the device edge on the top or bottom. This is done by calling `messageView.installBackgroundVerticalView(:insets:)`.
		/// See that method's documentation for details.
	...
	*/
	...
	public var preferredHeight: CGFloat = 350
	/**
...
extension SwiftMessagesSegue {
	private class TransitioningPresenter: NSObject, UIViewControllerAnimatedTransitioning {
		...
		func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
			...
			/// This works around a bug in iOS 11 where the safe area of `messageView` (and all ancestor views)
			/// is not set except on iPhone X. By assigning `messageView` to a view controller, its safe area is set consistently.
			/// This bug has been resolved as of Xcode 10 beta 2.
