...
open class SwiftMessages {
	...
	public struct Config {
		...
		/**
		 ...
		 `WindowController` is needed. Use this if you need to supply a custom subclass
		 of `WindowController`.
		 */
		public var windowController: ((_ windowLevel: UIWindow.Level?, _ config: SwiftMessages.Config) -> WindowController)?
	...
	fileprivate var _current: Presenter? {
		didSet {
			if oldValue != nil {
	...
	fileprivate func enqueue(presenter: Presenter) {
		if presenter.config.ignoreDuplicates {
			...
			if !queue.filter({ $0.id == presenter.id }).isEmpty { return }
		}
		func doEnqueue() {
	...
	fileprivate func dequeueNext() {
		guard _current == nil, !queue.isEmpty else { return }
	...
	fileprivate func queueAutoHide() {
		...
		if let pauseDuration = current.pauseDuration {
			...
			messageQueue.asyncAfter(deadline: delayTime) {
