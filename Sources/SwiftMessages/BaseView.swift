...
open class BaseView: UIView, BackgroundViewable, MarginAdjustable {
	 ...
	 */
	open func installContentView(_ contentView: UIView, insets: UIEdgeInsets = UIEdgeInsets.zero, preferredHeight: CGFloat = 350) {
		...
		if preferredHeight > 0 {
			contentView.heightAnchor.constraint(equalToConstant: preferredHeight).with(priority: UILayoutPriority(rawValue: 200)).isActive = true
		}
	}
	 ...
	 */
	open func installBackgroundView(_ backgroundView: UIView, insets: UIEdgeInsets = UIEdgeInsets.zero, preferredHeight: CGFloat = 350) {
		...
		if preferredHeight > 0 {
			backgroundView.heightAnchor.constraint(equalToConstant: preferredHeight).with(priority: UILayoutPriority(rawValue: 200)).isActive = true
		}
		layoutConstraints = [backgroundView.leftAnchor.constraint(equalTo: layoutMarginsGuide.leftAnchor, constant: insets.left).with(priority: UILayoutPriority(rawValue: 900)),
		...
		regularWidthLayoutConstraints = [backgroundView.leftAnchor.constraint(greaterThanOrEqualTo: layoutMarginsGuide.leftAnchor, constant: insets.left).with(priority: UILayoutPriority(rawValue: 900)),
	...
	/**
	 ...
	 - Parameter insets: The amount to inset the content view from the horizontal margins and vertical edges.
	   Default is zero inset.
	 */
	open func installBackgroundVerticalView(_ backgroundView: UIView, insets: UIEdgeInsets = UIEdgeInsets.zero, preferredHeight: CGFloat = 350) {
		backgroundView.translatesAutoresizingMaskIntoConstraints = false
		if backgroundView != self {
			backgroundView.removeFromSuperview()
		}
